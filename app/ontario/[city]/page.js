...
const page = async ({ params }) => {
  ...
  const url = `https://api.bridgedataoutput.com/api/v2/OData/itso/Property?access_token=f36a468ba28f13fc2076c91b1e6eaf13&$top=50&$orderby=ModificationTimestamp desc&$filter=PropertyType eq 'Residential' and City eq '${formattedSlug}'`;
  const options = {
    method: "GET",
  };
  const res = await fetch(url, options);
  const data = await res.json();
  const main_data = data.value;
      ...
      <div className="container-fluid">
        ...
        <div className="pt-3 row row-cols-md-4 ">
          {main_data.map((curElem) => {
            return (
              <ResoCard city={city} key={curElem.ListingId} curElem={curElem} />
            );
          })}
        </div>
      </div>
